...
const adapter = new adapter_pg_1.PrismaPg(new pg.Pool({ connectionString: process.env.DATABASE_URL }));
...
const CASES = [
    {
        ...
        status: client_1.Status.NEW,
        aiAnalysis: {
            caseType: 'Personal Injury',
            priority: client_1.Priority.HIGH,
            summary: 'Client was involved in a motor vehicle accident caused by another driver running a red light. Client sustained physical injuries resulting in inability to work and is experiencing insurance claim denial for medical expenses.',
            missingInformation: [
            ...
            recommendedAction: 'Schedule urgent consultation. Request police report, all medical records, insurance correspondence, and employment documentation for lost wages claim.',
    ...
    },
    {
        ...
        phone: '+13105559876',
        description: 'My employer has been making my work environment unbearable for the past six months since I reported a safety violation to HR. I have been passed over for two promotions I was qualified for, received negative performance reviews that contradict my previous excellent ratings, and was moved to a less desirable shift. I believe this is retaliation for my protected activity. I have documented everything and kept copies of my original positive performance reviews.',
        status: client_1.Status.REVIEWING,
        aiAnalysis: {
            caseType: 'Employment Law',
            priority: client_1.Priority.HIGH,
            summary: 'Client alleges workplace retaliation following a protected HR complaint about a safety violation. Pattern of adverse employment actions including denied promotions, negative performance reviews, and shift change suggest potential retaliation claim.',
            missingInformation: [
            ...
            recommendedAction: 'High priority case with strong documentation. Schedule consultation within 48 hours. Advise client to preserve all communications and avoid discussing case at work.',
    ...
    },
    {
        ...
        aiAnalysis: {
            ...
            recommendedAction: 'Review contract terms and payment schedule. Assess claims for breach of contract and recovery of overpayment. Evaluate potential for contractor license board complaint in addition to civil action.',
    ...
    },
    {
        ...
        aiAnalysis: {
            ...
            recommendedAction: 'Client has strong evidence for small claims court. Review applicable security deposit laws for jurisdiction. Calculate whether statutory damages and attorneys fees may be available.',
    ...
    },
    {
        ...
        aiAnalysis: {
            ...
            recommendedAction: 'Urgent. Opposing party already has counsel. Schedule consultation immediately. Review any documents already filed or served.',
...
async function main() {
    ...
    for (const caseData of CASES) {
        ...
        if (createdCase.status === client_1.Status.REVIEWING) {
            await prisma.auditLog.create({
                data: {
                    caseId: createdCase.id,
                    userId: adminUser.id,
        ...
        if (createdCase.status === client_1.Status.CONTACTED) {
            await prisma.auditLog.create({
                data: {
                    caseId: createdCase.id,
                    userId: adminUser.id,
        ...
        if (createdCase.status === client_1.Status.CLOSED) {
            await prisma.auditLog.create({
                data: {
                    caseId: createdCase.id,
                    userId: adminUser.id,
        ...
        console.log(`  ✅ Created case: ${caseData.clientName} (${aiAnalysis.caseType} - ${aiAnalysis.priority})`);
    }
